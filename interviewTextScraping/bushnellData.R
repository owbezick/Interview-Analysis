...
raw_lines <- pdf_text("Bushell.pdf") %>%
  read_lines()

# Text Preprocessing ----
# Extract interview Lines
raw_interview <- raw_lines[138:5636]
...
# Extract Answers
answers <- rm_between_multiple(interview_trimmed_white_space, left = 'BUSHNELL:', right = 'Q:', extract = T)
...
overall_sentiment_b <- answer_sentiment %>%
...
# Key Word Sentiment
key_words_chart_df_b <- key_words_df %>%
  summarise(anger = sum(anger)
            , disgust = sum(disgust)
            , fear = sum(fear)
            , joy = sum(joy)
...
all_answers_sorted_b <- answer_sentiment[order(answer_sentiment$overall_sentiment, decreasing = T),] %>% head(5)

key_words_df <- key_words_df %>%
  mutate(overall_sentiment = anger + anticipation + disgust + fear + joy + sadness + surprise + trust + negative + positive)

key_words_sorted_b <- key_words_df[order(key_words_df$overall_sentiment, decreasing = T),] %>% head(5)
